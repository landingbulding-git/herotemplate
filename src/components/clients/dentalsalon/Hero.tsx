...
export default function Hero() {
  ...
  return (
    <section className="relative min-h-screen flex flex-col justify-center bg-white overflow-hidden selection:bg-[#efc0b6]/30">
      
      {/* Background Gradient - Fixed and strengthened visibility */}
      <div 
        className="absolute inset-0 -z-10" 
        style={{ 
          background: 'linear-gradient(135deg, #efc0b6 0%, #ffffff 50%, #ffffff 100%)' 
        }}
      ></div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10 w-full pt-16 sm:pt-20">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
          
          {/* Left Side: Copy & Trust Stack */}
          <div className="lg:col-span-7 flex flex-col space-y-5 max-w-2xl mx-auto lg:mx-0 text-center lg:text-left">
            
            {/* Social Badge at the top */}
            <div className="flex items-center justify-center lg:justify-start gap-2 mb-2 animate-fade-in">
            ...
            {/* Headline - "Lowered" by adding pt-4 to container or space-y, and compressed for ATF */}
            <h1 className="text-3xl sm:text-4xl lg:text-[46px] font-black text-slate-900 tracking-tight leading-[1.1] pt-2">
              Tüntesd el a makacs nikotin- és kávéfoltokat maradéktalanul <span className="text-[#efc0b6] brightness-75">mindössze 45 perc alatt</span>, és nézz tükörbe újra azzal a büszkeséggel!
            </h1>
            ...
            <div className="hidden lg:flex items-start gap-3 pt-6 border-t border-slate-200/60">
               <div className="w-10 h-10 rounded-full bg-slate-200 shrink-0 overflow-hidden ring-2 ring-white">
               ...
               <div>
                 ...
                 <p className="text-[10px] font-black text-slate-900 uppercase tracking-widest mt-1">Knapek Éva <span className="text-slate-400 font-normal ml-1">szakpszichológus</span></p>
               </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-5 w-full max-w-md mx-auto relative lg:mt-0">
            ...
            {/* Infographic Badges */}
            <div className="absolute -left-6 top-8 z-20 hidden md:flex items-center gap-2 bg-white px-3 py-1.5 rounded-lg shadow-lg ring-1 ring-slate-100 animate-[bounce_4s_ease-in-out_infinite]">
            ...
            <div className="relative bg-white rounded-[2rem] shadow-2xl shadow-[#efc0b6]/40 ring-1 ring-slate-100 overflow-hidden flex flex-col z-10" id="booking">
              
              {/* Macro Image Concept - Scaled down for fold tightness */}
              <div className="h-32 relative bg-slate-900 overflow-hidden border-b-4 border-[#6fbaba]">
              ...
              <div className="px-6 py-6 sm:px-8 sm:py-8 bg-white">
                ...
                <form onSubmit={handleSubmit} className="space-y-3.5">
                   <input type="text" required placeholder="Név" className="block w-full px-4 py-3 border border-slate-200 rounded-xl bg-slate-50 placeholder-slate-400 focus:outline-none focus:bg-white focus:ring-2 focus:ring-[#6fbaba] text-sm font-medium transition-all" />
                   <input type="email" required placeholder="Email cím" className="block w-full px-4 py-3 border border-slate-200 rounded-xl bg-slate-50 placeholder-slate-400 focus:outline-none focus:bg-white focus:ring-2 focus:ring-[#6fbaba] text-sm font-medium transition-all" />
                   <input type="tel" required placeholder="Telefonszám" className="block w-full px-4 py-3 border border-slate-200 rounded-xl bg-slate-50 placeholder-slate-400 focus:outline-none focus:bg-white focus:ring-2 focus:ring-[#6fbaba] text-sm font-medium transition-all" />

                   <button
                     type="submit"
                     disabled={isSubmitting}
                     className="w-full flex justify-center items-center gap-2 py-3.5 px-4 rounded-xl shadow-xl shadow-[#6fbaba]/30 text-[14px] font-black text-white bg-[#6fbaba] hover:bg-[#5da9a9] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#6fbaba] transition-all hover:-translate-y-0.5 active:scale-95 uppercase tracking-wide"
                   >
                     {isSubmitting ? (
                ...
                <div className="mt-6 flex flex-col gap-2 bg-[#6fbaba]/5 p-3 rounded-xl border border-[#6fbaba]/20">
                  {[
